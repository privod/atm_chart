...
class DataFrameReaderOrcl(db.Orcl):
    ...
    def get_service(self, beg, end):
        ...
        # df.A_TIME_BEG = df.A_TIME_BEG.apply(lambda gp_time: typ.gp_to_time(str(gp_time)))
        # df.A_TIME_END = df.A_TIME_END.apply(lambda gp_time: typ.gp_to_time(str(gp_time)))
        df.A_TIME_BEG = df.A_TIME_BEG.astype(str).apply(typ.gp_to_time)
        df.A_TIME_END = df.A_TIME_END.astype(str).apply(typ.gp_to_time)
    ...
    def get_atm(self):
        ...
        df.MON_BEG = df.MON_BEG.astype(str).apply(typ.gp_to_time)
        df.MON_END = df.MON_END.astype(str).apply(typ.gp_to_time)
        df.TUE_BEG = df.TUE_BEG.astype(str).apply(typ.gp_to_time)
        df.TUE_END = df.TUE_END.astype(str).apply(typ.gp_to_time)
        df.WED_BEG = df.WED_BEG.astype(str).apply(typ.gp_to_time)
        df.WED_END = df.WED_END.astype(str).apply(typ.gp_to_time)
        df.THU_BEG = df.THU_BEG.astype(str).apply(typ.gp_to_time)
        df.THU_END = df.THU_END.astype(str).apply(typ.gp_to_time)
        df.FRI_BEG = df.FRI_BEG.astype(str).apply(typ.gp_to_time)
        df.FRI_END = df.FRI_END.astype(str).apply(typ.gp_to_time)
        df.SAT_BEG = df.SAT_BEG.astype(str).apply(typ.gp_to_time)
        df.SAT_END = df.SAT_END.astype(str).apply(typ.gp_to_time)
        df.SUN_BEG = df.SUN_BEG.astype(str).apply(typ.gp_to_time)
        df.SUN_END = df.SUN_END.astype(str).apply(typ.gp_to_time)
